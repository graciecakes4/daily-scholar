...
export default function QuizHistoryPage() {
  ...
  const getScoreColor = (percentage: number) => {
    if (percentage >= 80) return 'text-moss bg-moss/10';
    if (percentage >= 60) return 'text-gold-dark bg-gold/10';
  ...
  if (loading) {
    return (
      <div className="flex justify-center py-20">
        <div className="animate-pulse text-muted">Loading quiz history...</div>
      </div>
    );
  }
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div className="bg-paper-2 rounded-xl border border-rule p-4">
          <div className="text-2xl font-bold text-ink">{totalQuizzes}</div>
          <div className="text-sm text-muted">Quizzes Taken</div>
        </div>
        <div className="bg-paper-2 rounded-xl border border-rule p-4">
          <div className={`text-2xl font-bold ${avgScore >= 70 ? 'text-moss' : 'text-gold-dark'}`}>
            {avgScore.toFixed(1)}%
          </div>
          <div className="text-sm text-muted">Average Score</div>
        </div>
        <div className="bg-paper-2 rounded-xl border border-rule p-4">
          <div className="text-2xl font-bold text-gold">{totalQuestions}</div>
          <div className="text-sm text-muted">Questions Answered</div>
        </div>
        <div className="bg-paper-2 rounded-xl border border-rule p-4">
          <div className="text-2xl font-bold text-gold-dark">{perfectScores}</div>
          <div className="text-sm text-muted">Perfect Scores 🌟</div>
        </div>
      </div>
        ...
        <div className="bg-paper-2 rounded-xl border border-rule p-6">
          ...
          <div className="flex items-end gap-1 h-32">
            {quizzes.slice(0, 20).reverse().map((quiz, i) => (
              ...
              >
                <div
                  className={`w-full rounded-t ${quiz.percentage >= 80 ? 'bg-moss' :
                    quiz.percentage >= 60 ? 'bg-gold' : 'bg-rust'
                    }`}
      ...
      {quizzes.length === 0 ? (
        <div className="bg-paper border border-rule rounded-2xl p-12 text-center">
          <h2 className="text-xl font-semibold text-ink-2 mb-2">No Quizzes Yet</h2>
          <p className="text-muted">Complete quizzes from your daily learning to track your progress.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {quizzes.map((quiz) => (
            <div key={quiz.id} className="bg-paper-2 rounded-xl border border-rule overflow-hidden hover:shadow-md transition-all">
              <div className="p-4">
                <div className="flex items-center justify-between">
                    ...
                    <div>
                      <div className="flex items-center gap-2 flex-wrap">
                        {quiz.topics.map((topic, i) => (
                          <span key={i} className="px-2 py-1 bg-gold/10 text-gold-dark text-xs rounded">
                            {topic}
                          </span>
                        ))}
                      </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      ...
                      className="p-2 text-muted hover:text-rust"
                      title="Delete"
                    >
      ...
      {quizzes.length > 5 && avgScore < 70 && (
        <div className="bg-gold/5 border border-gold/30 rounded-xl p-6">
          <h3 className="font-bold text-gold-dark mb-2">💡 Study Tips</h3>
          <ul className="space-y-2 text-sm text-gold-dark">
      ...
      {quizzes.length > 5 && avgScore >= 85 && (
        <div className="bg-moss/5 border border-moss/25 rounded-xl p-6">
          <h3 className="font-bold text-moss mb-2">🎉 Great Job!</h3>
